...
load_env() {
  if [[ -f "$ENV_FILE" ]]; then
    while IFS= read -r line || [[ -n "$line" ]]; do
      ...
      if [[ ( "${value:0:1}" == "'" && "${value: -1}" == "'" ) || ( "${value:0:1}" == "\"" && "${value: -1}" == "\"" ) ]]; then
        ...
        case "$key" in
        MINIMAX_API_KEY|API_KEY|APP_MINIMAX_BASE_URL|APP_MINIMAX_MODEL|APP_MINIMAX_GROUP_ID|APP_PORT|SKIP_INGEST|APP_CORPUS_ROOT_PATH|APP_FAST_MODE_ROOT_PATH|APP_PDF_OCR_FALLBACK_ENABLED|APP_PDF_OCR_LANG|APP_EMBEDDING_ENABLED|APP_EMBEDDING_BASE_URL|APP_EMBEDDING_ENDPOINT|APP_EMBEDDING_MODEL|APP_EMBEDDING_TIMEOUT_SECONDS|APP_EMBEDDING_BATCH_SIZE)
          export "$key=$value"
          ;;
        APP_UI_INVITE_CODE)
          export "$key=$value"
  ...
  : "${APP_PORT:=8081}"
  : "${SKIP_INGEST:=true}"
  : "${APP_CORPUS_ROOT_PATH:=./corpus}"
  : "${APP_FAST_MODE_ROOT_PATH:=./fast-corpus}"
  ...
  : "${APP_EMBEDDING_BASE_URL:=http://localhost:8000}"
  : "${APP_EMBEDDING_ENDPOINT:=/v1/embeddings}"
  : "${APP_EMBEDDING_MODEL:=BAAI/bge-large-zh-v1.5}"
  ...
  (
    ...
    if command -v setsid >/dev/null 2>&1; then
      nohup setsid env \
        ...
        APP_FAST_MODE_ROOT_PATH="$APP_FAST_MODE_ROOT_PATH" \
        APP_PDF_OCR_FALLBACK_ENABLED="$APP_PDF_OCR_FALLBACK_ENABLED" \
        APP_PDF_OCR_LANG="$APP_PDF_OCR_LANG" \
        APP_EMBEDDING_ENABLED="$APP_EMBEDDING_ENABLED" \
        APP_EMBEDDING_BASE_URL="$APP_EMBEDDING_BASE_URL" \
        APP_EMBEDDING_ENDPOINT="$APP_EMBEDDING_ENDPOINT" \
        APP_EMBEDDING_MODEL="$APP_EMBEDDING_MODEL" \
        APP_EMBEDDING_TIMEOUT_SECONDS="$APP_EMBEDDING_TIMEOUT_SECONDS" \
        APP_EMBEDDING_BATCH_SIZE="$APP_EMBEDDING_BATCH_SIZE" \
